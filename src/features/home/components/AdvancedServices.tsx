...
export default function AdvancedServices({ dict, isRtl = true }: AdvancedServicesProps) {
    ...
    return (
        <section className="py-20 md:py-24 lg:py-32 bg-background overflow-hidden relative" id="professional-services">
            <div className="container mx-auto px-6 md:px-12">
                ...
                {/* Main Content Layout */}
                <div className="flex flex-col lg:flex-row gap-8 lg:gap-12 items-stretch lg:min-h-[550px]">
                    {/* Featured Image Area */}
                    <div className={cn(
                        "w-full lg:w-2/3 order-2 lg:order-2 relative rounded-[2rem] overflow-hidden shadow-2xl border border-white/5 bg-surface-container-low min-h-[450px]"
                    )}>
                        <AnimatePresence mode="wait">
                            ...
                            >
                                {activeItem.images.length === 5 ? (
                                    ...
                                    <div className="grid grid-cols-2 lg:grid-cols-4 grid-rows-4 lg:grid-rows-2 gap-2 h-full w-full p-2">
                                        <div className="relative rounded-xl overflow-hidden group/img">
                                            <Image src={activeItem.images[0]} alt={activeItem.title} fill className="object-cover transition-transform duration-700 group-hover/img:scale-110" />
                                        </div>

                                        {/* Smalls */}
                                        <div className="relative rounded-xl overflow-hidden group/img">
                                            <Image src={activeItem.images[1]} alt={activeItem.title} fill className="object-cover transition-transform duration-700 group-hover/img:scale-110" />
                                        </div>
                                        <div className="relative rounded-xl overflow-hidden group/img">
                                            <Image src={activeItem.images[2]} alt={activeItem.title} fill className="object-cover transition-transform duration-700 group-hover/img:scale-110" />
                                        </div>
                                        <div className="relative rounded-xl overflow-hidden group/img">
                                            <Image src={activeItem.images[3]} alt={activeItem.title} fill className="object-cover transition-transform duration-700 group-hover/img:scale-110" />
                                        </div>
                                        <div className="relative rounded-xl overflow-hidden group/img col-span-2 row-span-2 lg:col-span-4 lg:row-span-1">
                                            <Image src={activeItem.images[4]} alt={activeItem.title} fill className="object-cover transition-transform duration-700 group-hover/img:scale-110" />
                                        </div>
                                    </div>
                                ) : (
                                ...
                                {/* Content Overlay */}
                                <div className={cn(
                                    "absolute inset-x-0 bottom-3 md:bottom-4 lg:bottom-5 px-4 md:px-6 lg:px-8",
                                    ...
                                    <motion.div
                                        ...
                                        className="max-w-2xl"
                                    >
                                        <h3 className="inline-block text-2xl md:text-4xl font-bold text-white mb-3 font-headline tracking-tight leading-[1.12] bg-black/35 px-4 md:px-5 py-2 rounded-xl backdrop-blur-sm">
                                            {activeItem.title}
                                        </h3>
                                        <p className="inline-block text-white/95 text-sm md:text-base font-normal leading-relaxed mb-4 line-clamp-2 bg-black/28 px-4 py-2 rounded-xl backdrop-blur-sm">
                                            {activeItem.desc}
                                        </p>
                                        <div className={cn("flex flex-wrap gap-3", isRtl ? "justify-start" : "justify-start")}>
                                            {activeItem.highlights.map((highlight, hIdx) => (
                                                <span
                                                    key={hIdx}
                                                    className="px-3 py-1.5 rounded-lg border border-white/20 bg-black/30 backdrop-blur-sm text-white text-[10px] md:text-xs font-semibold tracking-wide uppercase"
                                                >
                                                    {highlight}
                                                </span>
                                            ))}
                    ...
                    <div
                        className={cn(
                            "hidden w-full lg:flex lg:w-1/3 lg:flex-col lg:py-6 relative lg:order-1"
                        )}
